...
OUT  = os.path.join(HERE, 'ver_' + TAG)
...
EPS_FR4 = 4.5
PX, PY = 4.0, -6.3            # hot-loop centre
CX, CY = -1.22, -28.97        # input terminal block (KF2EDGV-3.81-2P)
F_MIN, F_MAX = 30e6, 1e9

# ---- B2 additions ----
CABLE_L  = 100.0              # 10 cm cable
CABLE_W  = 1.0                # conductor cross-section [mm] (~0.98 mm eq. dia.)
YE       = CY - CABLE_L       # far (supply) end of the cable
CM_GAP   = 0.5                # lumped-port gap (y direction) in the cable
Cd       = 0.5                # lumped-element gap height [mm]
Z_GP     = z_bot - 10.0       # reference ground plane (10 mm below board)
GPX      = (-88.0, 88.0); GPY = (-140.0, 30.0)   # plane (176 x 170 mm, reduced)
...
NrTS = 40000 if FAST else 120000
...
loop.AddBox(priority=13, start=[Xa, PY-0.15, z_top], stop=[PX, PY+0.15, z_top])
loop.AddBox(priority=13, start=[PX+GAP, PY-0.15, z_top], stop=[Xb, PY+0.15, z_top])
for xx in (Xa, Xb):
    loop.AddBox(priority=13, start=[xx-0.15, PY-0.15, z_bot], stop=[xx+0.15, PY+0.15, z_top])
...
by = list(np.arange(Y0, Y1+1e-9, resb)) + [Y0, Y1, PY, CY, PY-0.15, PY+0.15, YE,
...
mesh.SmoothMeshLines('all', 22.0 if FAST else 18.0, 1.5)
