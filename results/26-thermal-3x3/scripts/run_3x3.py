#!/usr/bin/env python3
"""TPS62933 board thermal 3x3 driver (geometry x load).
- builds one mesh per geometry (main + coarse for convergence)
- ElmerGrid convert + body/BC detection
- h(DeltaT) natural-convection outer iteration per case
- metrics (board mean, device temps, U21 near-field ref), conservation
- writes numbers_thermal_3x3.json
Usage: run_3x3.py [mesh|all]
"""
...
OX, OY = 6.223, 33.736
KEEPOUT = [(OX-3.2999934, OY-3.499993,  OX+3.2999934, OY+3.499993),
           (OX+7.9838042, OY-17.272,    OX+12.3361958, OY-12.7098044)]
...
def build_mesh(geo, level):
    ...
    r = subprocess.run(["python3", "build_mesh_3x3.py", str(lmax), str(lmin), geo, out],
                       capture_output=True, text=True, timeout=1200)
    if r.returncode != 0:
        print(r.stdout[-2000:], r.stderr[-2000:]); raise SystemExit("mesh fail " + out)


def setup_elmer(geo, level, bodies_json="bodies.json"):
...
def detect_bc(mdir):
    ...
    for b, c in bcen.items():
        X, Y = c[0]*1e3, c[1]*1e3
        if abs(c[2]) < Lz/2 * 0.9:
            if any(x0 <= X <= x1 and y0 <= Y <= y1 for (x0, y0, x1, y1) in KEEPOUT):
                cutout.append(int(b))
            else:
...
def build_case(geo, level, Iout, h, Tamb, bc, casedir, thscale=1.0, kz=KZ_VIA):
...
def main():
    ...
    numbers = {}
    npath = os.path.join(ROOT, "numbers_thermal_3x3.json")
    ...
    # convergence: coarse 3A vs main 3A (2 levels) per geometry
    conv = {}
    for geo in GEOS:
    ...
    json.dump(conv, open(os.path.join(ROOT, "convergence_3x3.json"), "w"), indent=1)
    # full 3x3 on main mesh
    for geo in GEOS:
        for Iout in LOADS:
